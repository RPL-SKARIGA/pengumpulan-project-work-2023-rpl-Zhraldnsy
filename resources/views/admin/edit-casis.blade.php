    ...
    <div id="page-content-wrapper">
        <nav class="navbar navbar-expand-lg navbar-light bg-transparent py-4 px-4">
            <div class="d-flex align-items-center">
                ...
                <h2 class="fs-2 m-0 primary-text">Proses Data Calon Siswa</h2>
            </div>
        ...
        <div class="container">
          <form action="/casis-update/{{$data->id}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="row mt-2 bg-transparent ">
                ...
                <div class="form-check mb-4 col-sm-2 text-white">
                    <input class="form-check-input" type="radio" name="gender" id="gender" value="L"  @if ($data->gender == 'L')
                    checked
                    @endif>
                ...
                <div class="form-check mb-4 col-sm-4 text-white">
                    <input class="form-check-input" type="radio" name="gender" id="gender" value="P"  @if ($data->gender == 'P')
                    checked
                    @endif>
                    <label class="form-check-label" for="gender">
                        Perempuan
                ...
                <div class="mb-4 col-sm-4 jurusan">
                    <select class="form-select jurusan" aria-label="Default select example" name="pilihan_jurusan_1">
                         @foreach($jurusan1 as $jurusan1)
                            <option value="{{ $jurusan1->id }}" {{ $data->jurusan1->id === $jurusan1->id ? 'selected' : '' }}>
                                {{ $jurusan1->nama_jurusan }}
                            </option>
                        @endforeach 
                    </select>
                </div>
                ...
                <div class="mb-4 col-sm-4 jurusan">
                    <select class="form-select jurusan" aria-label="Default select example" name="pilihan_jurusan_2">
                        ...
                        <option value="{{ $jurusan2->id }}" {{ $data->jurusan2->id === $jurusan2->id ? 'selected' : '' }}>
                            {{ $jurusan2->nama_jurusan }}
                        </option>
                    @endforeach 
                    </select>
                </div>
                ...
                <div class="mb-4 col-sm-4 eskul">
                    <select class="form-select eskul" aria-label="Default select example" name="pilihan_eskul_1">
                        @foreach($eskul1 as $eskul1)
                        <option value="{{ $eskul1->id }}" {{ $data->eskul1->id === $eskul1->id ? 'selected' : '' }}>
                            {{ $eskul1->nama_eskul }}
                        </option>
                    @endforeach 
                    </select>
                ...
                <div class="mb-4 col-sm-4 eskul">
                    <select class="form-select eskul" aria-label="Default select example" name="pilihan_eskul_2">
                        @foreach($eskul2 as $eskul2)
                        <option value="{{ $eskul2->id }}" {{ $data->eskul2->id === $eskul2->id ? 'selected' : '' }}>
                            {{ $eskul2->nama_eskul }}
                        </option>
                    @endforeach                     </select>
                </div>
            ...
            <div class="d-flex">
                ...
                <a href="/casis-delete/{{$data->id}}" class="btn btn-outline-danger btn-lg rounded-pill mt-2 mb-3 w-100 primary-text">Tolak</a>
                <a href="/konfirmasicasis/{{$data->id}}" class="btn btn-outline-success btn-lg rounded-pill mt-2 mb-3 w-100 primary-text">Terima</a>
            </div>
            </form>
        </div>
